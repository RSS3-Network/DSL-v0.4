	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for index := range got {
				...
				if err := json.Unmarshal(got[index].Metadata, &g); err != nil {
					t.Error(err)
				}
				if err := json.Unmarshal(tt.want[index].Metadata, &w); err != nil {
					t.Error(err)
				}
			...
			// check other field
			for index := range got {
				got[index].Metadata = []byte("")
				tt.want[index].Metadata = []byte("")
			}
